...
当前开发版本：**v1.6.1（初步修改版，非稳定发布）**｜最近稳定标签：**v1.6.0**｜
作者：**Suzuran Ye**｜平台：**Windows**
...
v1.6.1 统一了 6 个六字页面入口和按钮尺寸。扫描页先用两个大按钮选择完整或快速，再展开
对应设置；Full 可分别启用 ExifTool 与 ffprobe。格式、容器和 RAW 解码检查全部移入独立
“档案数据核验”，由五个同排按钮分别启用，不提供抽样滑块。环境页逐项显示六项能力状态，
并把五个安装按钮保持在同一排。下拉模式也修复了重复选择当前项后文字变白的问题。本版
没有升级数据库 schema；旧 `full-scan`／`quick-scan` 参数和 v1.4.1 只读兼容边界保持不变。

v1.6.1 当前用于收敛 UI 与交互设计，已通过仓库自动化验收，但仍可能存在真实工具、特殊
文件和不同 Windows 显示环境下尚未发现的问题。本轮只发布 `Codex` 分支，不建立 tag，
不把该版本声明为稳定版；需要稳定基线时仍应使用 `v1.6.0`。
...
| 档案扫描建库 | 选择完整／快速模式，建立可暂停恢复的快照 | schema 4 SQLite；必要时 Issues Markdown |
| 档案快照对比 | 比较 schema 3／4 快照，区分增删、变化、移动和证据不足 | Diff SQLite；必要时 Issues Markdown |
| 档案数据核验 | 组合内置、ExifTool、ffprobe、7-Zip 与 RAW 深检 | 分板块 Markdown／JSON；RAW 伴随报告 |
| 档案数据解析 | 识别数据库模块，选择内容与输出格式 | 自包含 HTML、中文 XLSX、CSV、JSONL、manifest |
...
- 完整扫描默认计算完整 SHA-256，提取规范化元数据并保留 raw payload；扫描页只负责建库，
  不提供格式校验或 RAW 深检。
- 完整扫描可分别关闭 ExifTool 或 ffprobe 元数据采集；两个工具默认开启。主动关闭导致的
  未处理文件记为“跳过”而非文件错误。
...
会先返回完整界面。依赖安装后的自动环境复检完成后再恢复设置，硬盘检测则返回专用的
硬盘选择布局。
...
- 内置校验：ZIP／OOXML 成员 CRC 与 PDF 基础结构；
- ExifTool：适用图片、RAW 和媒体文件的 `-validate`；
- ffprobe：GIF、视频和音频的容器／流结构；
- 7-Zip：压缩包和旧 Office OLE 容器；
- RAW 深检：由独立 rawpy／LibRaw 子进程实际解码。

五项默认关闭，启用后检查该工具全部适用文件，不提供范围滑块；不支持或无法识别的格式
只计总数，不列为文件问题。当前文件基础状态核对不依赖这些按钮。核验可暂停、继续或停止，
但核验报告本身没有扫描 partial，因此不提供跨次保存续传。rawpy 不可用时 RAW 按钮禁用
并显示原因；解码失败、截断或 timeout 才进入问题报告。
...
当前没有 `v1.6.1` tag，也没有对应的稳定源码归档。若需要固定稳定版本，请使用
`git checkout v1.6.0`；试用初步版前建议先记录当前 `Codex` 提交哈希。
...
顶部 6 个功能入口保持同一排，并统一为 6 个字。进入「档案扫描建库」时不会偷偷默认
Full；必须先选择“完整扫描模式”或“快速扫描模式”，随后才显示该模式的设置。二元选项
默认使用固定尺寸按钮，开启为绿、关闭为黄；也可在顶部设置切换为下拉菜单。

顶部「设置」可持久化调整默认窗口大小、字体、字号、二态选项的按钮／下拉样式和“任务
完成提示音”。提示音默认关闭；正常完成或“完成但需要检查”时异步播放一次，
失败、暂停、保存退出、停止、依赖安装和硬盘检测准备阶段不播放。任务运行期间关闭窗口
始终要求确认；空闲关闭直接保存配置并退出，不再提供额外确认开关。

DAISY 恢复最后使用的功能页面、白名单内非路径选项和已确认的手动工具路径；不保存档案、
数据库、输出目录或硬盘选择。偏好写入 `Output/GUI_Settings.json`；损坏或非法字段按项回退
到安全默认值，`Output` 不进入 Git。
...
DAISY 仅支持 Windows，v1.6.1 使用 Python 3.14 验证。
...
ENV 页先用一排 6 个状态按钮显示 ExifTool、ffprobe、7-Zip、PowerShell、smartctl 和
rawpy，再用一排 5 个等宽按钮提供 ExifTool、ffprobe、7-Zip、smartctl 与 rawpy／LibRaw
安装。每次安装都需要再次确认；外部工具只通过固定 WinGet 白名单处理，rawpy 只通过当前
Python 环境的固定 pip 白名单处理。PowerShell 不由该页面安装。手动工具路径统一位于
「高级 > 工具路径」。
...
- 旧库缺少 schema 4 能力时显示 `NULL／不可用`，不伪造为 0、空或成功；旧库前后摘要、
  大小和 mtime 必须不变。
- 兼容保证是“v1.6.1 读取 v1.4.1 封存库”，不保证 v1.4.1 程序读取 schema 4，也不允许
  v1.6.1 直接接管 v1.4.1 未完成 partial。
...
发布测试计划和逐阶段证据见
[v1.6.1 实施计划](Spec/Spec_DAISY_V1_6_1_Implementation_Plan.md)与
[v1.6.1 测试记录](Spec/Spec_DAISY_V1_6_1_Test_Record.md)。v1.6.0 的数据与恢复设计仍见
[v1.6.0 实施计划](Spec/Spec_DAISY_V1_6_0_Implementation_Plan.md)。
