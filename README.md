...
顶部菜单栏按“环境”“数据库”“硬盘”提供三个任务下拉菜单，不再使用左侧
工作台。`ENV-11 完整扫描` 因生成快照数据库而列入数据库菜单，编号保持不变；
`DB-21` 同属数据库菜单。硬盘菜单为后续存储设备模块预留 `STG-` 前缀，
...
标准菜单提供环境／数据库／硬盘任务下拉菜单，以及项目目录、结果目录、退出、
面板显示、关于信息与 GitHub 主页入口；命令预览默认关闭并可由“视图”菜单
打开。右侧滚动条使用低对比度米黄色滑块，不再跟随任务主题变色。按钮悬停
会显示用途说明，
