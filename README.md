...
随后在「ENV-01 环境检测」或「DB-11 完整扫描」的高级选项中选择对应的 `.exe`。CLI
...
首次建立正式基准时，在 GUI 中选择“DB-11 完整扫描”并保留默认值：
...
| DB-11 | 完整扫描／`full-scan` | 生成完整 SQLite 快照，支持断点续传 |
| DB-12 | 快速扫描／`quick-scan` | 只登记树、大小、时间和可选 File ID |
...
| DB-91 | 数据库自检／GUI 维护入口 | 运行随附 unittest，重点验证数据库与关键工作流；不读取私人档案或生成正式产物 |

顶部菜单栏按“环境”“数据库”“硬盘”提供三个任务下拉菜单，色带下方另有
可折叠的任务按钮菜单，两套入口同步当前选中项，不再使用左侧工作台。下拉
菜单按任务性质加入分隔线、主题色悬停和当前项高亮。`DB-11 完整扫描` 与
`DB-12 快速扫描` 因生成快照数据库而列入数据库菜单，之后依次为 `DB-21`、
`DB-31`、`DB-32`、`DB-41` 和维护编号 `DB-91`。其中 `11/12` 表示快照采集，
...
日志固定排列在进度下方，不再依赖可被缩窗挤没的分隔窗格。页内品牌栏与
窗口左上角 logo 均移除，标准菜单下方直接显示三色色带。
标准菜单提供带分组与选中高亮的环境／数据库／硬盘任务下拉菜单；色带下方
提供可折叠、可换行的同步任务按钮菜单。标准菜单还提供项目目录、结果目录、
退出、面板显示、关于信息与 GitHub 主页入口；命令预览默认关闭并可由“视图”
菜单打开。右侧滚动条使用低对比度米黄色滑块，不再跟随任务主题变色。按钮悬停
会显示用途说明，
目录／日志／缓存等辅助操作与开始／停止任务控制分为上下两组，辅助操作会
随可用宽度自动换行。GUI 不再注册自定义快捷键，页面右上角的
只读／产物提示徽标也已移除。
...
也可以进入 GUI 的“DB-91 数据库自检”页点击“运行数据库自检”。它调用同一套
...
业务功能；缺少测试文件时，“DB-91 数据库自检”页的运行按钮会禁用。
...
反馈问题前请先运行“ENV-01 环境检测”和“DB-91 数据库自检”，并说明 Windows、Python 和
