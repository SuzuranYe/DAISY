...
左侧以顶部彩带的绿、黄、红分别标出“环境”“数据库”“硬盘”三区，各区可
独立折叠；所有任务项的选中态统一使用绿色。`ENV-11 完整扫描` 因生成快照
数据库而显示在数据库区，编号保持不变；`DB-21` 同属数据库区。硬盘区为
后续存储设备模块预留 `STG-` 前缀，但 v1.4.2 不创建空任务页，也不提前加入
任何硬盘业务功能。
...
下方，不再依赖可被缩窗挤没的分隔窗格。页内品牌栏与窗口左上角 logo 均
移除，标准菜单下方直接显示三色色带；
