...
窗口左上角 logo 均移除，标准菜单下方直接显示低饱和浅蓝绿／浅蓝／浅紫色带。
标准菜单提供带分组与无虚线描边的浅色选中提示；色带下方
提供可折叠的同步功能模块区，`ENV`、`DBS`、`STG` 固定各占一行；所有功能块
使用相同尺寸和四字名称，不随窗口宽度重排，普通窗口也不能缩得比完整功能模块
更窄；完整编号与名称保留在悬停说明中。各任务中的哈希抽样比例统一归入高级设置。
标准菜单还提供项目目录、
...
也可以进入 GUI 的“DBS-91 数据库自检”页点击“运行数据库自检”。它调用同一套
