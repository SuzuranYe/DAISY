...
- 当前顶栏固定为 `文件｜功能｜视图｜设置｜高级｜帮助`，菜单栏使用浅米黄色底色，
  字体族和字号与正文同步。「功能」按「档案」「设备」「维护」分组，顺序与色带下方
  「扫描建库、快照对比、数据核验、数据解析、硬盘登记、环境检测」六个四字入口一致。
  「设置」包含窗口大小、界面字体、工具路径、完成提示音、结果目录提示和恢复设置；
  「高级」只包含扫描选项、核验选项、对比选项、动态显示／隐藏预览和功能自检。扫描选项只设置超时处置和
...
- 顶部功能入口使用四字短名，设置页、进度、日志和报告使用完整功能名；6 个入口始终保持
  单排，并采用与表单模式按钮相同的平直外形。入口使用浅米黄色系、固定宽高比例并整组
  靠左，不随窗口横向拉伸；普通态使用暖米黄色，选中态再加深一级，不得改变按钮尺寸。
  GUI 中，值域严格为
...
- 普通按钮使用统一字符宽度、内边距和 12 px 水平间距；文件资源管理器按钮使用较小专用
  ...
  状态按钮均重新执行完整环境检测。两排按钮使用固定紧凑宽度、保持相同宽高并靠左排列，
  不再横向拉伸填满整行。
...
- 「结果目录提示」默认关闭。普通业务任务产出有效结果后，只将底部「打开结果目录」按钮
  短暂切换为绿色提示样式，再自动恢复，不抢焦点；启用该设置后才额外询问是否打开目录。
  底部操作按「开始任务 → 暂停／保存并退出（适用时）→ 停止 → 打开结果目录」排列，使用
  同一宽度、内边距和字号。「重置会话」位于顶部功能栏标题区，运行期间禁用。
...
右边界；文件资源管理器小按钮统一使用「添加」「选择」「浏览」两个字，并置于内容左侧。
界面不显示必填星号，
...
- 小窗视图在空闲和运行时始终可进入；「视图」菜单提供动态「小窗模式／完整界面」，
...
- 顶部「设置」菜单持久化窗口大小、字体族、字号、完成提示音、结果目录提示、
...
| `result_directory_prompt_enabled` | `false` | 任务产出结果后是否询问打开目录；关闭时仍短暂突出结果按钮 |
...
- 安装队列完成后 GUI 刷新当前进程 PATH，并通过统一能力探测层重新检测外部工具和
  ...
  退出码宣称升级成功。
- 顶部功能栏的「重置会话」先要求确认，再清除项目内白名单缓存目录
  `__pycache__`、`.pytest_cache`、`.mypy_cache`、`.ruff_cache`，
  独立 `.pyc`/`.pyo` 文件与当前窗口工具路径缓存，同时清空所有表单参数、
  目录队列、硬盘清单、日志和三条进度，并返回运行环境检测页；窗口、字体、提示音和
  结果目录提示等软件设置不变。
