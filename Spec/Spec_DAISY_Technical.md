...
- 状态：**v1.6.4 长期生产版本规范**。
- 当前长期生产版本与标签：**v1.6.4**。
- 生产版本谱系：v1.4.1 是第一版，v1.6.4 是第二版；「生产版本」表示适合长期部署并作为
  后续兼容维护基线，不否定其间普通稳定发布的历史事实。
...
v1.6.4 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.4`。
...
点击「全选」选择所有联机且信息完整的设备；「全选／取消选择」复用「浏览」按钮的
`FilePicker.TButton` 样式、字符宽度和高度。每块已选硬盘拆成独立
...
硬盘登记页的「管理员模式」使用与「收起设置／展开进度」相同的字符宽度、内边距和
实际尺寸，并与左侧「管理员权限已启用／未启用」标题在同一 Y 轴居中；其左边缘与设置
标题栏倒数第二个「恢复默认」按钮对齐。
...
- 当前应用版本为 `1.6.4`。统一 `scan` 新产物使用 `schema_version=4`、
...
- 「结果目录弹窗」默认关闭。普通业务任务产出有效结果后，只将底部米黄色的
...
- 普通窗口以 `1600×900` 为默认目标尺寸；设置中的 3 个预设按分辨率升序排列为
  `1366×768`、`1600×900`、`1920×1080`。Windows 进程启用 Per-Monitor V2
...
- v1.6.2 的发布验收覆盖多字体、3 档字号、1.0/1.25/1.5 Tk scaling，以及
  ...
  1366×768、1600×900、1920×1080。标准字号和 `1600×900` 默认窗口设置下，6 个正式页面
