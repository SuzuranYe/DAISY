...
- 状态：**v1.6.9 开发规范**。
- 当前开发版本：**v1.6.9**；当前稳定生产版本与最新正式 Git 标签：**v1.6.8**；
  历史兼容稳定生产版本：**v1.4.1**。
- 稳定生产基线：**v1.4.1 → v1.6.8**。v1.6.4 与 v1.6.6 曾被指定为长期稳定基线，
  但均因 GUI 哈希功能确认失效而撤销该定位；v1.6.7 修复相关致命问题，现由 v1.6.8
  作为当前稳定生产版继承。
...
- 当前只认定 v1.6.8 与 v1.4.1 为稳定生产版本。v1.6.8 用于当前部署；v1.4.1 仅作为
  历史兼容稳定版和 schema 3 最低只读兼容基线。v1.6.1 是未打标签的阶段性修改，不作为
