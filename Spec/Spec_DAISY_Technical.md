...
- v1.4.2 定位为纯 UI 优化版本：增加只保留进度与停止控制的小窗运行视图；多项任务
  ...
  日志可独立折叠，日志固定置于进度下方。取消左侧工作台，顶部菜单栏按
  “环境”“数据库”“硬盘”提供三个任务下拉菜单；`ENV-11 完整扫描` 与
  `DB-21` 均归入数据库菜单，但保留既有编号。后续存储设备模块预留 `STG-`，
  硬盘菜单只显示未开放提示，不创建空任务页。
  页内品牌栏与窗口左上角 logo 均移除，标准菜单下方直接显示三色色带；同时
  移除自定义快捷键和任务页提示徽标。标准菜单提供目录、退出、
  视图、关于与 GitHub 入口，命令预览默认关闭；顶部任务菜单在运行期间锁定，
  右侧滚动条使用固定的低对比度米黄色滑块。目录、日志、缓存和工具安装归入
  可换行的辅助操作行，开始／停止固定置于独立任务控制行。
