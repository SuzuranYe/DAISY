...
- 所有功能模块按钮与设置页标题共用同一套六字名称。开发 GUI 中，值域严格为
  `{False, True}` 的表单项统一使用二态按钮：黄色表示关闭，按下后绿色表示启用；
  完整／快速、抽样／全量、预设和 timeout 处置等多值模式仍使用下拉菜单。根标签映射
  直接显示为文本输入；既有 `--force`、`--map-root` 默认值和执行语义不变。只读下拉框
  未展开时拦截鼠标滚轮，滚轮只滚动页面，不改变选项。
...
|---|---|
| ENV-01 | 环境报告目录 |
| 扫描 | 扫描模式、启动方式、档案根目录、生成方式、（续传快照）、快照目录、（元数据范围）、NTFS标识、（哈希模式）、（上一封存快照）、（根标签映射）、（重试范围） |
| 对比 | 基准快照、对比快照、差异目录、根标签映射、指纹降级 |
| 核验 | 封存快照、档案根目录、哈希核验、格式校验、（RAW深检）、报告目录 |
