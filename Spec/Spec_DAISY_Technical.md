...
- 发布沿革：v1.6.1 仅为未打标签的阶段性修改，v1.6.2 首次收敛该阶段能力，v1.6.3
  ...
  并把上述成果收敛为长期生产基线；v1.6.5 在不改变数据契约的前提下维护界面可见性、
  控件对齐和说明文案。
...
v1.6.5 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.5`。
...
- 当前应用版本为 `1.6.5`。统一 `scan` 新产物使用 `schema_version=4`、
