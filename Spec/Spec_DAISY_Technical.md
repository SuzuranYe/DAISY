...
- 状态：**v1.6.9 长期稳定生产规范**。
- 当前长期稳定生产版本：**v1.6.9**；历史兼容长期稳定生产版本：**v1.4.1**。
  当前仓库最新正式 Git 标签仍为 **v1.6.8**，v1.6.9 的本地稳定提交尚未创建发布标签。
- 长期稳定生产基线：**v1.4.1 → v1.6.9**。v1.6.4 与 v1.6.6 曾被指定为长期稳定基线，
  但均因 GUI 哈希功能确认失效而撤销该定位；v1.6.7 修复相关致命问题，v1.6.8 完成界面与
  核验收敛，现由 v1.6.9 作为当前长期稳定生产版继承。
- 发布沿革：v1.6.1 仅为未打标签的阶段性修改，v1.6.2 首次收敛该阶段能力，v1.6.3
  继续完成 UI、默认窗口、文案和交互一致性整理；v1.6.4 修正核验工具的环境检测门控，
  并曾把上述成果标为长期稳定基线；v1.6.5 维护界面可见性、控件对齐和说明文案；
  v1.6.6 收敛界面细节、文档和内部冗余；v1.6.7 修复 GUI 哈希工作进程无法启动、
  进程退出异常及默认功能自检可能拖垮桌面的致命问题；v1.6.8 增加无数据库核验、
  数据库原路径直用、Python 环境管理，并统一主要按钮和目录池；v1.6.9 统一哈希抽检／复核
  术语，明确末次复扫边界，并调整运行结束后的面板和持续提示行为。
...
右下角任务操作使用同一请求尺寸和无描边平面样式。环境页的 7 个检测／安装项使用相同按钮宽度，
...
| 扫描后独立重算 | 哈希抽检 | 扫描阶段按冻结比例抽样，由 PowerShell 独立重算 |
| 数据核验中的重新计算 | 哈希复核 | 按核验范围重新计算并与数据库中的有效 SHA-256 基准比较 |
| DAISY 自带的 ZIP/OOXML/PDF 校验 | 基本校验 | GUI 名称；内部键仍为 `verify_builtin`，不包含 ExifTool、ffprobe、7-Zip 或 rawpy/LibRaw |
| 档案数据解析范围 | 全量、摘要、自定义 | 内部值仍为 `full-audit`、`human-summary`、`custom` |
...
| 哈希、schema 4 隔离工作进程、超时、复用、哈希抽检与复核 | `Script\Lib\Script_DAISY_Lib_File_Hash.py` |
...
- 采集 NTFS-ID；
- 哈希抽检默认抽样 1%，至少 100 个本次实际计算且有效的条目。
...
沿用其中冻结的扫描模式、根目录、哈希、逐工具元数据范围、NTFS-ID、哈希抽检、格式和
...
4. 执行 SQLite `integrity_check` 与 `foreign_key_check`；
...
点击「开始」，程序不得自动读取或自动继续。普通「停止」也保留未完成快照，但写入
...
6. 元数据阶段只处理 `pending`。当前实现不写 `meta_status=processing`；未提交事务由
   ...
   重试；
7. 重新执行末次 size/mtime 复扫、独立哈希抽检和封存；事件 JSONL 以追加方式记录
...
完整扫描的独立哈希抽检和 `check-hash` 使用 PowerShell `Get-FileHash`，与主哈希实现分离。schema 4
扫描的哈希抽检按文件启动本任务持有的精确 PowerShell 进程，路径以 UTF-8 令牌放入
...
双方恢复为原摘要才算偶发抽检异常，否则写入 `verify_hash` attempt，并把当前哈希标为
unstable 留证。
...
v1.6.9 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
- 其他格式返回 `unsupported`；只有另行启用哈希复核时，内容变化证据才由哈希层提供。
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.9`。
...
未匹配 SMART 读取目标的设备保留在池中并显示原因，但不可选择。用户只可逐项点击选择；
选中项使用全局普通有效选项的鼠尾草整行底色与暖白文字，不显示勾选框。已有至少一块硬盘
被选中时，继续增减硬盘只更新当前行、命令预览和开始按钮，不重建设置表单；只有选择数量在
零与非零之间变化、需要显隐结果输出章节时才允许重建。界面不提供「全选／取消选择」。
独立的「检测硬盘」按钮位于池子上方和边框外，硬盘内容列在
下方固定 `1 px` 灰褐描边池内。每块已选硬盘拆成独立
`队列 i/n` 子进程和独立 ZIP；即使只选一块也显示 `队列 1/1`。每次点击「检测硬盘」
都会先清除上一轮清单与选择。接入状态改变后必须重新检测，
不得沿用旧 `DiskNumber`。
...
- 当前应用版本为 `1.6.9`。统一 `scan` 新产物使用 `schema_version=4`、
...
- 当前只认定 v1.6.9 与 v1.4.1 为长期稳定生产版本。v1.6.9 用于当前部署；v1.4.1 仅作为
  历史兼容长期稳定版和 schema 3 最低只读兼容基线。v1.6.8 是已被取代的正式稳定版本，
  不再列入当前长期稳定基线。v1.6.1 是未打标签的阶段性修改，不作为
  发布版本；v1.6.2 虽曾通过当时的自动化回归、v1.4.1 FULL 兼容专项与发布审计并建立正式
  标签，但其 GUI 哈希与默认功能自检问题后来得到确认，因此不再具有稳定生产定位。
  自动化验收不替代真实工具、特殊文件、超大档案和不同 Windows 设备的持续验证。
- 经现有 Git 提交与标签定位，v1.5.0 是过渡版本；没有证据表明它包含后来确认的真实 Tk
  默认自检风险或 GUI 哈希控制管道故障。v1.5.1 的默认功能自检会反复创建真实 Tk 窗口，
  存在桌面严重卡顿或卡死风险，但当时尚未引入 GUI 哈希控制管道故障。
- v1.6.0、未正式发布的 v1.6.1 及 v1.6.2～v1.6.6 均继承上述默认功能自检风险；同时，
  GUI 完整扫描始终保持控制管道打开，导致 Windows `spawn` 哈希工作进程无法就绪，
  SHA-256 采集功能实际失效。无控制管道的直接 CLI 哈希路径不属于该失效范围。v1.6.4
  与 v1.6.6 原有的长期稳定定位据此撤销。
...
- 档案数据解析提供快照 15 个数据模块、Diff 6 个数据模块，状态只来自统一 Reader；
  「全量／摘要／自定义」三种导出范围的内部值分别为
  `full-audit`、`human-summary`、`custom`，并与 HTML/XLSX/CSV/JSONL 格式相互独立。只有
  ...
  模块按钮使用固定宽高，不随窗口边缘伸缩，只按可用宽度换行；固定按钮列和间距列保证各行
  精确对齐，模块区四边留白一致。
...
- `Script_DAISY_Lib_Scan_Runtime.py` 的 schema 4 内部生产链依次执行枚举、哈希、元数据、可选格式、复扫、
  独立哈希抽检、读取性能分析、封存和发布。扫描专用 `verify_format` 明确记为 skipped，
...
- 当前顶栏固定为 `文件｜功能｜视图｜设置｜高级｜帮助`，菜单栏使用暖白底色，
  字体族和字号与正文同步。「功能」按「档案」「设备」「环境」分组，顺序与三色分界线上方
  ...
  当前文件显示；哈希抽检比例固定为默认值，不在 GUI 中提供调节入口。
...
- 顶部功能入口与设置页、进度、日志和报告统一使用六字完整功能名；6 个入口始终保持
  ...
  正文和固定宽高，不使用描边，并与页面右下角常驻任务按钮统一尺寸；整组靠左，不随窗口横向拉伸，
  普通态和选中态的变化不得改变按钮尺寸。
  GUI 中，值域严格为
  `{False, True}` 的表单项固定使用二态按钮：灰褐表示关闭，鼠尾草表示启用；顶部
  设置不提供控件样式切换。扫描页的完整／快速使用两个
  固定模式按钮；页面默认完整显示并选择完整扫描、全新生成、分别生成及全部采集内容。完整扫描
  的元数据只显示 ExifTool/ffprobe 两个独立
  按钮，各自在「全量 → 基础 → 关闭」之间循环。逐工具
  全量范围只控制既有 `raw_payloads.provider`，不改变 DDL。
  数据核验可在「已有数据库／无数据库」间切换，默认选择「已有数据库／路径未变」，进入页面
  即显示封存快照、核验项目和报告目录；
  路径状态决定直接采用快照原路径，还是显示根目录映射池。切换为无数据库时显示独立目录池，并禁用没有
  基准可比较的哈希复核。一次核验只接受一份基准数据库，但该合并快照可登记最多 9 个根目录，
  因此路径变化映射池和直接目录池均保留最多 9 项。核验项目使用哈希复核、基本校验、ExifTool、ffprobe、7-Zip、
  ...
  能力恢复后仍按该状态选择；新配置默认请求除 rawpy/LibRaw 外的全部项目。启用项目全量检查适用文件。
  续传重试范围使用
  「仅未处理／瞬时失败／全部未成功」三个互斥按钮；超时处置使用高级菜单中的单选项；
  档案数据解析的导出范围及 HTML/XLSX/CSV/JSONL 输出格式均使用固定按钮，不显示复选框；
  数据库识别成功后默认「全量」并选中全部可选择模块和四种格式。RAW 深度校验受统一能力探测
  约束。对比页默认完整显示基础设置并选择「单目录」；只有选择「多目录」才展开根目录名配对。
  更换任一快照必须恢复「单目录」并清除旧配对。每组「基准根目录名／对比根目录名」
  按上下两行编辑；「添加配对」位于
...
- 普通按钮使用统一字符宽度、内边距和 12 px 水平间距；输出目录的「浏览」使用较小专用
  尺寸。目录与根目录池的「添加」、数据解析的「解析数据库」及硬盘登记的「检测硬盘」使用
  科研蓝高对比工作流样式；输入数据库「选择」使用天青输入样式。上述页内工作流按钮的宽度、
  高度、字体和内边距必须与「完整扫描」标准按钮完全一致，重要程度只通过角色配色表达，不得
  额外放大。顶部六个功能入口和右下角三个常驻任务按钮使用独立的较大常驻控件规格，不受此条
  影响。运行环境检测页按
  「检测状态 → 安装与更新 → 检测报告 →（环境报告目录）」排列；报告默认不导出，只有开启后
  才显示目录并向 CLI 传递 `--export-report`。
  检测状态和安装与更新共用 7 个对齐位置及完整工具名，包含 Python、ExifTool、ffprobe、
  7-Zip、PowerShell、smartctl 和 rawpy/LibRaw；6 个安装操作可点击。可安装项第一行
  显示工具名，第二行统一显示「安装或更新」；PowerShell 对应列第一行显示工具名，第二行
  显示「手动管理」并禁用，说明 DAISY 不负责其安装或更新。等待检测为灰色，成功为绿色且
  第二行显示实际版本，不可用为红色；点击任一状态按钮均重新执行完整环境检测。Python 使用
  当前运行解释器的版本与路径，
  不得在 GUI 缓存层被过滤；PowerShell 状态卡与本机摘要只显示主次版本。缺失工具或可选能力
  属于检测完成后的警告结果，必须列出具体缺失项，不得显示为环境检测失败。两排按钮与
  「完整扫描」等模式按钮同宽，
  高度约为其 1.5 倍以容纳两行文字，并靠左按最多四列整项换行，不横向拉伸填满整行。
- 需要改变条件字段时，GUI 先在未映射的新 `Frame` 中完成控件、字体和几何构建，再原子替换
  `Canvas` 的当前表单并销毁旧 `Frame`；旧模式按钮不得先绘制中间选择态。建库方式、解析预设
  等不改变字段布局的按钮只原位保存值和更新模块／预览。核验方式切换必须保留滚动位置和标题
  位置，不能出现先跳变再重建。普通字段仅设置独立最小行高，不得加入跨字段统一行高组；核验
  项目等可换行控件只能增加自身所在行的高度，不得同步撑开其他普通字段。
- 档案数据解析在没有数据库路径时只显示输入数据库及天青「选择」；路径已选但尚未成功识别时
  只增加科研蓝「解析数据库」；识别成功后一次显示导出范围、数据模块、输出格式和数据导出
  目录，默认「全量」并选择全部可选择模块和四种格式。自定义预设必须至少选择一个数据模块，
  输出格式至少选择一种。数据库「选择」位于第一行，完整路径输入框
  独占第二行；不得让长路径与操作按钮在同一行竞争宽度。识别失败时后续内容继续隐藏；识别
  成功后，即使自定义模块或输出格式尚未选完整，导出设置也保持可见，只让底部「开始」禁用。
  点击「解析数据库」不得改变设置、进度或日志的展开状态；离屏重建表单时必须保持数据库路径
  和滚动位置，不得出现先清空后逐项闪现。模块按钮使用固定尺寸，按钮列与 12 px 间距列分离，
  最后一行也必须与前面各行对齐。
- 六个主功能必须按真实依赖自上而下组织：扫描和快照对比默认完整显示常用设置；核验默认按
  「已有数据库／路径未变」完整显示，并允许自上而下切换数据库状态和路径状态；数据解析先选择
  并解析数据库，识别成功后一次展示全部导出设置；硬盘登记先检测、再选盘、最后显示输出设置。
  分阶段表单重建前后必须按触发字段的屏幕纵坐标复位，不得因内容总高度变化而移动已经点击的
  「已有数据库／路径未变」等选择行。环境检测没有前置用户输入，不得为追求形式而增加无意义
  的确认步骤。任何必需输入未完成时，右下角「开始」必须为灰色禁用态。
...
- 「结果目录弹窗」默认关闭。普通业务任务产出有效结果后，只将底部卡其色的
  「产出」按钮在常态与绿色提示态间持续交替，不抢焦点，直到用户点击该按钮；启用该设置后
  才额外询问是否打开目录。
  该按钮常态使用卡其，顶部六个未选中功能入口使用天青，二者均无描边。
  底部操作从右向左固定为「开始／停止 → 暂停／继续 → 产出」。三个常驻按钮与顶部六个功能
  入口使用同一套较大尺寸；日志标题区工具按钮使用统一的小型面板工具规格。同一个「开始」按钮在任务运行时原位
  变为橙色「停止」；扫描进入暂停请求态后，才在暂停按钮左侧显示「保存并退出」，暂停按钮
  原位改为「继续」，但二者在后端确认安全暂停前均保持禁用。未使用独立的可见停止按钮。
  「重置软件」位于顶部功能栏操作区，运行期间禁用；「设置」菜单不再提供重复恢复入口。
- 设置正文初始展开，运行进度和运行日志正文初始收起。任务开始时收起设置并展开进度和日志；
  任务队列成功、需检查、失败、停止或保存并退出后立即重新展开设置并同时收起运行进度与
  运行日志；小窗任务先返回完整界面。「展开日志」随后在中性常态与天青提示态间持续交替，
  直到用户点击该按钮。依赖安装的中间安装进程不单独切换面板，自动环境复检真正结束后再执行
  同一规则；硬盘检测完成时也遵守该规则。
- 扫描队列全部以退出码 0/1 完成且没有 `failed`、`failed_recoverable`、`stopped` 或
  `save_exit` 结果时，GUI 原位清空本次「档案根目录」池并禁用尚未重新选择输入的开始按钮；
  该操作只清除窗口输入，不删除源目录或任务产物。失败、停止、保存并退出及未启动时保留根
  目录输入，以便诊断、重试或续传。
- 扫描续传提示卡位于设置标题行下方，不得占用标题右侧的设置操作列；目录、核验根目录映射、
  快照配对和硬盘列表使用 `1 px` 灰褐边框的内容池。添加／检测按钮全部位于池子边框外，内容在
  下方池内逐行显示，空池也保留占位说明。扫描与核验的添加目录、快照对比的添加配对使用
  科研蓝，数据解析的数据库选择使用天青、数据库解析使用科研蓝；快照输入和输出目录使用
  中性的「浏览」。档案根目录池中每行的移除「×」固定置于序号和完整路径之前。硬盘池
  不显示「全选／取消选择」。
...
| 档案数据核验 | 核验方式、（路径状态）、（封存快照）、（档案根目录）、核验项目、核验报告目录 |
...
自 v1.6.8 起，权威配色源为《明日方舟_孤星_配色取样_修复版_v2.pptx》。GUI 只允许使用下列
核心配色与可选配色的精确数值；语义别名必须指向表内数值，不得保留旧色、近似取样值或新增
未登记的深浅色阶。
...
可选配色来自用户补充色板，不改变核心色的语义，只负责大面积中性色、信息分层和状态区分：

| 补充色 | 色值 | 当前用途 |
|---|---|---|
| 暖白 | `#F0E7E2` | 窗口底、中性按钮与进度／日志内容面 |
| 科研蓝 | `#5E8CC0` | 工作流主操作与三条普通进度 |
| 鼠尾草 | `#647B75` | 设置顶部区域与普通有效选项 |
| 灰褐 | `#A89F98` | 功能模块底、字段／内容池及面板边界 |
| 板岩 | `#2C3240` | 进度／日志标题栏 |
| 旧粉 | `#D3998B` | 错误状态浅底 |
| 墨青 | `#1F272A` | 日志正文 |
| 天青 | `#80AFC3` | 顶部入口、暂停、输入选择与日志选区 |
| 卡其 | `#C5B778` | 产出操作 |
| 石灰 | `#A9A397` | 中性按钮悬停态与进度槽 |
| 暗橙 | `#DE5123` | 管理员模式主操作与停止按钮悬停态 |

大面积底色不得直接铺满浅黄字或先驱青绿。功能模块容器不显示外框，所有按钮均采用无描边
平面样式；字段、内容池、设置卡、进度和日志面板仍保留结构边界。批准色白名单同时约束 GUI
和档案数据解析生成的 HTML 报告；生产代码不得内联表外色值。按钮状态只通过填充色和
文字体现。功能模块整体使用灰褐底色，不显示「功能模块」重复标题；4 px 三色细带位于功能
模块与下方内容之间，只承担分界作用。最左功能按钮到模块区左、上、下三边的距离与相邻按钮
间距均为 12 px。设置区分割线上方全部使用鼠尾草底和暖白文字，分割线下方使用浅色表面，
三者必须可直接区分。顶部模块常态为天青、当前项为板岩，底部开始／暂停／
产出分别使用科研蓝／天青／卡其，常态状态徽标使用板岩。管理员权限未启用时使用浅黄区域和
暗橙主按钮，启用后使用先驱青绿区域和鼠尾草状态按钮；管理员区域不额外叠加包围线。

宽度为 12 字符的浏览、选择、重置、展开／收起及日志工具按钮统一增加纵向内边距，不单独保留
一组矮按钮。顶部功能栏右侧只保留「重置软件」；功能栏、设置、进度与日志标题区的右侧操作组
对齐到同一可见右边界。设置、进度与日志操作组使用 12 px 四边等距外边距，最右展开／收起按钮
到上、下、右三边及左侧相邻按钮的距离一致。页内「添加／选择／解析数据库／检测硬盘」全部
使用「完整扫描」的标准尺寸：输入选择为天青，工作流执行为科研蓝，只通过颜色表达角色和重要
程度。顶部六个入口和右下角三个常驻按钮使用同一套较大尺寸。普通有效选中态统一为鼠尾草底、
暖白字；明确的「关闭／不采集／不输出」统一为灰褐底、墨青字、石灰悬停；尚未选择的普通候选
项使用暖白底和墨青字。元数据全量／基础／关闭保持相同字体与字重，不再各占一种色相；文件
标识不采集、哈希不采集、核验项目关闭、解析模块和输出格式不输出必须复用同一关闭态。
保存目录、其他单行输入框和真正承载内容的池统一使用 `1 px` 灰褐
边界；纯操作说明行不得再套一层边框，避免同页出现视觉上不同粗细的重复矩形。

按钮角色必须遵循下表。同一角色必须复用相同尺寸、字体、字重、内边距和色板；同一重要程度
不得因页面不同产生另一套设计。只有输入、执行、结果、危险或状态等明确语义不同，才允许改变
填充色：

| 按钮角色 | 几何规格 | 常态配色 | 适用操作 |
|---|---|---|---|
| 顶部功能入口 | 较大常驻规格、10 号常规字重 | 天青；当前项板岩 | 6 个六字功能入口 |
| 右下角常驻任务 | 与顶部入口同一较大规格 | 开始科研蓝、暂停天青、产出卡其、停止信号橙 | 开始／停止、暂停／继续、产出 |
| 未就绪开始 | 与右下角常驻任务相同 | 石灰灰底＋中性灰字 | 必需选择未完成或输入尚未识别 |
| 页内输入选择 | 完整扫描标准尺寸、10 号粗体 | 天青＋墨青字 | 输入数据库「选择」 |
| 页内工作流执行 | 完整扫描标准尺寸、10 号粗体 | 科研蓝＋暖白字 | 添加、添加配对、解析数据库、检测硬盘 |
| 普通有效选项 | 标准尺寸、10 号常规字重 | 鼠尾草＋暖白字 | 已选择的模式、模块或输出格式 |
| 明确关闭选项 | 与普通选项完全相同 | 灰褐＋墨青字 | 关闭、不采集、不输出 |
| 中性定位／面板工具 | 12 字符小型规格、10 号常规字重 | 暖白＋深色正文 | 浏览、重置、展开／收起、日志工具 |
| 特殊语义操作 | 对应区域的既定几何规格 | 信号橙／暗橙或状态色 | 危险停止、管理员提权与状态 |

禁止重新引入无人引用的旧按钮样式，或在具体页面直接复制一套与上述角色等价但色值、尺寸、
字体或字重不同的配置。

- 功能栏在完整界面中固定显示；设置区、进度区和日志区可独立显示或隐藏。设置区默认展开，
  进度区和日志区默认收起。
  任务设置、进度、日志和命令区采用固定 grid 顺序，不提供拖动调整；点击「开始」后自动收起
  设置、展开进度和日志，日志获得 1080p 剩余纵向空间。进度折叠而设置与
  日志展开时，日志正文增加等于进度正文的高度；用户手动形成该组合时，日志接管原进度
  区域。命令预览
  默认关闭。队列总进度、当前任务阶段和本阶段工作量三条进度语义独立；顶部显示
  当前完整路径。运行进度与运行日志使用相同的板岩标题栏、暖白标题文字和暖白内容面；
  三条进度统一使用科研蓝，进度槽统一使用石灰。队列、任务阶段和本阶段的
  标题、说明与百分比在普通状态统一使用板岩文字；成功、警告或失败结束态再让三行整体切换为
  同一语义色。三行均固定保留百分比位置；确定进度显示整数百分比，当前阶段无法计算时显示
  「计算中」，结束后恢复并保留数值百分比。日志可打开为
  单例独立窗口，与主窗口实时追加和清空同步。
- 小窗视图在空闲和运行时始终可进入；「视图」菜单以固定的「设置区／进度区／日志区」
  复选项表示面板可见性，可见即勾选，并提供动态「小窗模式／完整界面」，
...
|---|---|---|---|
| 空闲进入页面 | 展开 | 收起 | 收起 |
| 点击「开始」 | 收起 | 展开 | 展开并占用剩余高度 |
| 任务运行结束 | 立即展开 | 立即收起 | 立即收起；「展开日志」持续闪烁至点击 |
| 开始检测硬盘 | 收起 | 展开 | 展开 |
| 硬盘检测成功 | 立即展开并刷新硬盘选择 | 立即收起 | 立即收起；按钮持续提示 |
| 硬盘检测失败 | 立即展开，硬盘清单不更新 | 立即收起 | 立即收起；按钮持续提示诊断日志 |
| 点击解析数据库 | 保持，只显示数据库入口和解析操作 | 保持 | 保持 |
| 解析数据库成功 | 保持并显露导出范围、模块、格式和目录 | 保持 | 保持 |
| 解析数据库失败 | 保持后续设置隐藏，「开始」保持禁用 | 保持并保留诊断 | 保持并保留诊断 |
...
| 复扫／哈希抽检 | 条目累计，最多 500 ms 一次 | 当前文件开关关闭时不发送路径事件 |
...
- 六个功能页的首个内容分区必须显示非重复标题：扫描设置、对比输入、核验输入、解析输入、
  登记准备、检测状态。首区即使只有一个字段也保留分区标题；后续分区只在包含多个字段且
  不与首字段重名时显示独立标题，单字段分区直接使用字段标题，避免重复文案占用 1080p
  的纵向空间。分阶段展开不得改变已经显示的首区标题或首字段位置。
- v1.6.2 的历史发布验收曾覆盖多字体、3 档字号、1.0/1.25/1.5 Tk scaling，以及
  ...
  验收由 AI 按需手动打开软件执行。
  v1.6.8 人工验收覆盖 `1920×1080` 默认目标及 `1600×900`、`1366×768`、`1280×960`
  三种客户区目标，并检查数据解析、扫描建库、快照对比、数据核验、硬盘登记、环境检测及
  设置／进度／日志组合状态。顶部 6 个入口必须保持单排，因此 1024 px 不声明为正常支持宽度；
  普通界面最小宽度为 1180 px。客户区窄于 1500 px 时，入口从 16 字符请求宽度切换为 12 字符
  请求宽度并继续压缩横向留白，六字标签仍不得缩写、裁切或换行。
...
| `result_directory_prompt_enabled` | `false` | 任务产出结果后是否询问打开目录；关闭时产出按钮仍持续闪烁至点击 |
...
- 「档案数据解析」先识别快照或 Diff，再按统一 Reader 能力选择数据模块。快照最多 15 个数据模块，
  Diff 最多 6 个数据模块；没有输入时只显示数据库选择，选定路径后才显示独立的「解析数据库」
  操作行，解析成功后一次显露全部导出设置。「选择」与「解析数据库」同为标准按钮尺寸，分别使用
  天青输入色和科研蓝执行色。GUI 将内容选择命名为「导出范围」和「数据模块」，范围用按钮选择，
  数据模块按钮固定靠左对齐；解析操作不得改变页面展开状态或滚动位置。导出范围默认「全量」，
  全部可选择模块及 HTML/XLSX/CSV/JSONL 四种格式默认选中；切换为摘要按预设确定模块，切换为
  自定义后才允许逐项调整模块。切换导出范围不得覆盖已经选择的格式；
  `human-summary`/`full-audit`/`custom` 与输出格式保持正交。
...
- 「帮助」依次提供「关于」「联系作者」「GitHub 主页」；「关于」
  显示应用版本、当前扫描输出与兼容读取快照各自的数据库结构版本、元数据配置、
...
- GUI 默认优先使用 `Microsoft YaHei UI`，并可在本机已安装的中文／系统候选字体间
  ...
  功能栏右侧唯一的「重置软件」与设置、运行进度和运行日志标题区的工具按钮统一使用小型
  面板工具尺寸；页内
  「添加／选择／解析／检测」使用标准按钮尺寸，两类不得因重要程度再额外放大。
  同一操作行使用相同字体、内边距、列间距和右侧基线。运行进度和运行日志右侧操作组采用
  12 px 四边等距外边距；设置区右侧操作组遵循相同规则，最右展开／收起按钮四周的可见空隙一致。
  设置、进度和日志的展开／收起按钮只改变对应内容区，标题栏水平边距和按钮 x 轴位置保持
  不变。经典 Tk 与 ttk 的按钮、勾选按钮、单选按钮和菜单按钮均关闭虚线焦点绘制；ttk 样式
  布局移除 `focus` 元素，鼠标命令结束后仅在焦点仍停留于所点控件时交还该控件所在窗口，
  不得夺走命令主动交给输入框的焦点。顶部不显示「功能模块」标题，
  三色装饰线保留为 4 px 细线并移动到功能栏与下方内容之间。系统标题栏使用 16/32/48 px
  小雏菊多尺寸图标；界面不再
  显示 DAISY 花体字标。
...
- 硬盘信息登记检测开始时自动展示进度与日志。检测结束后立即重新展开设置、收起进度与日志，
  并让「展开日志」持续闪烁至点击；成功时刷新硬盘池并弹窗，失败时保留诊断日志和原硬盘清单。
...
- Python 已可运行时，「运行环境检测」会同时报告 Python、已发现工具的本机版本和
  ...
  「手动管理」占位；上下两区都按最多四列整项换行。所有按钮都使用完整工具名，等待时第二行显示
  「等待检测」，成功时直接显示实际版本，不使用笼统的「可用」，缺失时显示「不可用」；
  PowerShell 只在状态卡和本机摘要中显示主次版本，完整版本仍可保留在结构化报告。环境清单
  含缺失工具或不可用可选能力时，进程使用退出码 1 表示「检测完成、需要检查」，GUI 状态和
  日志必须写明具体缺失项，不得写成环境检测失败。JSON 检测报告默认不导出；只有用户开启
  「检测报告」后才创建目录和文件，缺失场景也应生成完整清单。开启导出但报告无法写入时
  返回退出码 2，并明确写出报告导出失败；不得把它降级为普通缺失项。
