...
- 状态：**v1.6.3 本地开发检查点规范**。
- 当前远端稳定版本与标签：**v1.6.2**；v1.6.3 尚未推送或建立标签。
- 发布沿革：v1.6.1 仅为未打标签的阶段性修改，v1.6.2 为当前稳定版，v1.6.3 继续整理 UI。
...
GUI 顶部使用 6 个四字短入口，并按档案工作流、设备、维护的顺序排列；设置页、进度和报告
...
| 对象 | 统一写法 | 说明 |
|---|---|---|
| 六个主功能 | 运行环境检测、档案扫描建库、档案快照对比、档案数据核验、档案数据解析、硬盘信息登记 | 页面标题与功能入口使用完整名称；准备动作使用本表所列名称 |
...
| NTFS 文件身份 | NTFS-ID | 由 NTFS 卷序列号与文件索引组成；数据库字段和历史文档中的 `File ID` 指同一证据 |
| 扫描后独立重算 | 哈希复检 | 「抽样」只描述选择比例，不作为该功能的页面名称 |
| 档案数据解析范围 | 摘要内容、全部内容、自定义 | 内部值仍为 `human-summary`、`full-audit`、`custom` |
| 档案数据解析项目 | 数据模块 | 不使用含义重叠的「导出模块」或「内容模块」 |
| 数据模块状态 | 可导出、0 条记录、无可用记录、版本不兼容、结构异常 | 0 条记录表示模块存在但为空；无可用记录表示数据库没有可解释的对应证据 |
...
| 能力检测状态 | 等待检测、可用、不可用 | 「等待检测」不等于「不可用」；「不可用」也不等于「未安装」 |
...
4. **业务运行纯本地**：所有数据库与存储业务任务均没有网络、遥测、上传或在线查询；云占位文件不会被触发下载。只有用户明确确认后，Python 引导脚本、GUI 外部工具 WinGet 安装或 rawpy pip 安装流程才可能联网。
...
v1.6.3 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.3`。
...
无法读取 SMART 信息的设备保留在池中并显示原因，但复选框禁用。用户可逐项勾选，也可
点击「全选」选择所有联机且信息完整的设备。每块已选硬盘拆成独立
...
- 当前应用版本为 `1.6.3`。统一 `scan` 新产物使用 `schema_version=4`、
...
- 当前顶栏固定为 `文件｜功能｜高级｜设置｜视图｜帮助`，菜单栏使用浅米黄色底色。
  「功能」包含「环境」「数据」和「硬盘」三个子菜单；色带下方固定一排显示 6 个等宽
  功能按钮。「高级」包含「工具路径」「扫描行为」「核验行为」「对比行为」、
  动态「显示／隐藏命令预览」命令和「DAISY 功能自检」。扫描行为只设置超时处置和
...
- 所有功能模块按钮与设置页标题共用同一套六字名称；6 个入口始终保持单排，使用和表单
  模式按钮相同的平直外形。入口采用浅米黄色系、固定宽高比例并整组靠左，不随窗口横向
  拉伸；普通态使用暖米黄色，选中态再加深一级，不得改变按钮尺寸。GUI 中，值域严格为
  ...
  数据核验使用内置格式校验、ExifTool、ffprobe、7-Zip、rawpy/LibRaw 五个
  完整名称同排按钮，能力可用的项目默认开启并全量检查适用文件。续传重试范围继续使用
  下拉菜单；超时处置使用高级菜单中的单选项；档案数据解析的导出范围及
  HTML/XLSX/CSV/JSONL 输出格式均使用固定按钮，不显示复选框。
  RAW 深度校验受统一能力探测约束。对比页用「基准根目录名 → 对比根目录名」左右配对编辑器
  表达根目录名对应关系；「添加配对」位于配对列表上方。对比的指纹降级位于「高级→对比行为」；
  既有 `--force`、`--map-root` 默认值和执行语义不变。只读下拉框未展开时拦截鼠标滚轮，滚轮
  只滚动页面，不改变选项。
- 普通按钮使用统一字符宽度、内边距和 12 px 水平间距；文件资源管理器按钮使用较小专用
  尺寸，顶部六入口按普通按钮宽高同比放大。所有按钮文字统一使用与正文相同的 10 号标准字号和
  常规字重；按钮层级只由尺寸、颜色和位置表达，不单独放大或加粗文字。运行环境检测页按
  「检测状态 → 软件安装 → 环境报告目录」排列，
  检测状态和软件安装共用六个对齐列及完整工具名；五个安装操作可点击。可安装项第一行
  显示工具名，第二行统一显示「安装或更新」；PowerShell 对应列第一行显示工具名，第二行
  显示「系统提供」并禁用。等待检测为灰色，可用为绿色，不可用为红色；点击任一
  状态按钮均重新执行完整环境检测。两排按钮保持相同宽高。
...
- 任务队列成功、需检查、失败、停止或保存并退出后重新展开设置；结束时若处于小窗模式，
  先返回完整界面。依赖安装后的自动环境复检属于连续流程，不在两段任务之间展开；硬盘
  检测成功则继续使用展开设置、收起进度与日志的专用选择布局。
...
|---|---|
| 运行环境检测 | 环境报告目录 |
| 档案扫描建库 | 扫描模式、生成方式、档案根目录、建库方式、（续传快照）、快照保存目录、元数据（ExifTool/ffprobe 独立三态）、NTFS-ID、哈希 (SHA-256)、（重试范围） |
| 档案快照对比 | 基准快照、对比快照、对比结果目录、根目录名配对；高级菜单提供指纹降级 |
| 档案数据核验 | 封存快照、档案根目录、核验项目、核验报告目录 |
| 档案数据解析 | 输入数据库、导出范围、数据模块、输出格式、数据导出目录 |
| 硬盘信息登记 | 硬盘选择、硬盘档案目录、简化报告 |

顶部「高级→工具路径」直接使用 ExifTool、ffprobe、7-Zip、PowerShell 和 smartctl 的
完整名称，并在同一行显示「手动」「已检测」或「尚未检测」状态；扫描、核验和对比行为
分别置于独立子菜单。六个功能页说明各用一句短句直接描述用途，不写
...
| 开始检测硬盘 | 收起 | 展开 | 展开 |
| 硬盘检测成功 | 展开并刷新硬盘选择 | 收起 | 收起 |
| 硬盘检测失败 | 展开，硬盘清单不更新 | 展开并保留诊断 | 展开并保留诊断 |
| 点击解析数据库 | 收起 | 展开 | 展开 |
...
- 普通窗口以 `1920×1080` 为目标尺寸。Windows 进程启用 Per-Monitor V2
  ...
  最小值和功能模块宽度。工作区不足时优先完整留在当前显示器内。标准字号下的 1080p 默认布局
...
- v1.6.2 的发布验收覆盖多字体、3 档字号、1.0/1.25/1.5 Tk scaling，以及
  1840×1020、1440×900、1280×720、1280×960、1280×1024、1100×850；设置菜单另覆盖
  1920×1080、1600×900、1366×768。标准字号和默认窗口设置下，1080p 的 6 个正式页面
  均无可见滚动；较小窗口
  或特大字号只有在内容超过视口与 2 px 几何容差后才显示滚动条，并可到达最后一项。
  固定按钮 UI 的正式页面交叉矩阵为 2 种本机候选字体 × 3 种 scaling × 3 档字号 ×
  3 种几何 × 6 个页面，共 324 组；更广窗口集合另行遍历全部现行与兼容 TaskSpec。
  顶部 6 个入口必须保持单排，因此 1024 px 不声明为正常支持宽度；普通界面最小宽度为
  1100 px，窄窗口只压缩按钮内部横向留白，不缩写、裁切或换行六字标题。该矩阵只证明
  当前仓库自动化结果，不等同于真实设备全覆盖。

### 12.5 用户配置、关闭与管理员重启

- 顶部「设置」菜单持久化默认窗口大小、字体族、字号、任务完成提示音、
...
- 无 Python 时，`Script\Script_DAISY_Install_Python.ps1` 只在用户确认后
  通过固定包 ID 安装或更新 Python 3.14，不安装其他工具。
- 该引导脚本保持 ASCII 英文提示，以兼容 Windows PowerShell 5.1 对 UTF-8 无 BOM 脚本的
  旧式解码行为；这不改变项目正式文本输出使用 UTF-8 无 BOM、LF 的要求。
- Python 已可运行时，「运行环境检测」会同时报告已发现工具的本机版本和
  全部不可用项。页面先用一排六个等尺寸状态按钮显示 ExifTool、ffprobe、7-Zip、
  PowerShell、smartctl 和 rawpy/LibRaw，再用相同六列显示五个安装操作及 PowerShell
  「系统提供」占位；上下按钮的横坐标和宽度一致。所有按钮都使用完整工具名，状态文字使用
  「等待检测／可用／不可用」，不使用 `EXIF`/`FF`/`PS`/`smart`/`RAW` 等简称；两行文字
  同时设置整体居中和逐行居中，不依赖系统 ttk 主题的默认排版。外部工具
...
- ExifTool、ffprobe、7-Zip、PowerShell、smartctl 的手动可执行文件路径只在
  顶部「高级→工具路径」菜单统一指定，并优先于本窗口检测缓存和运行时查找。
- 安装队列完成后 GUI 刷新当前进程 PATH，并通过统一能力探测层重新检测外部工具和
  ...
  退出码宣称升级成功。
- 「重置会话」先要求确认，再清除项目内白名单缓存目录
  `__pycache__`、`.pytest_cache`、`.mypy_cache`、`.ruff_cache`，
  独立 `.pyc`/`.pyo` 文件与当前窗口工具路径缓存，同时清空所有表单参数、
  目录队列、硬盘清单、日志和三条进度，并返回运行环境检测页；窗口、字体和提示音等
  软件设置不变。
...
DAISY 功能自检」可启动同一套测试，覆盖 SQLite schema、数据库约束、快照、Diff、
