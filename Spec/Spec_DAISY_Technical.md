...
一组矮按钮。顶部功能栏、设置、进度与日志标题区的右侧操作组对齐到同一可见右边界；设置、
进度与日志操作组使用 12 px 四边等距外边距，最右展开／收起按钮
...
- 功能栏、设置区、进度区和日志区可独立显示或隐藏，任务设置、运行进度与运行日志默认
  收起。任务设置、进度、日志和命令区采用固定 grid 顺序，不提供拖动调整；点击
  点击「开始」后自动收起设置、展开进度和日志，日志获得 1080p 剩余纵向空间。进度折叠而设置与
...
- GUI 默认优先使用 `Microsoft YaHei UI`，并可在本机已安装的中文／系统候选字体间
  ...
  功能栏、设置、运行进度和运行日志标题区的工具按钮统一使用小型面板工具尺寸；页内
  ...
  各展开／收起按钮只改变对应内容区，标题栏水平边距和按钮 x 轴位置保持
