...
- 顶部功能入口与设置页、进度、日志和报告统一使用六字完整功能名；6 个入口始终保持
  ...
  数据库识别成功后默认「全量」并选中全部可用模块和四种格式。RAW 深度校验受统一能力探测
...
- 普通按钮使用统一字符宽度、内边距和 12 px 水平间距；输出目录的「浏览」使用较小专用
  ...
  影响。运行环境检测页按
  「检测状态 → 软件安装 → 检测报告 →（环境报告目录）」排列；报告默认不导出，只有开启后
  ...
  红色；点击任一状态按钮均重新执行完整环境检测。Python 使用当前运行解释器的版本与路径，
...
- 档案数据解析在没有数据库路径时只显示输入数据库及天青「选择」；路径已选但尚未成功识别时
  ...
  目录，默认「全量」并选择全部可用模块和四种格式。自定义预设必须至少选择一个数据模块，
...
- 「档案数据解析」先识别快照或 Diff，再按统一 Reader 能力选择数据模块。快照最多 15 个数据模块，
  ...
  全部可用模块及 HTML/XLSX/CSV/JSONL 四种格式默认选中；切换为摘要按预设确定模块，切换为
