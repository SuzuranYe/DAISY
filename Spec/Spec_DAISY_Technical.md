...
- 状态：**v1.6.6 长期稳定版规范**。
- 当前长期稳定版本：**v1.6.6**；最近已有 Git 标签为 **v1.6.5**。
- 长期稳定谱系：**v1.4.1 → v1.6.4 → v1.6.6**。v1.6.5 保留为普通界面维护版本，
  不再列入长期稳定谱系。
...
v1.6.6 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
- 当前应用版本为 `1.6.6`。统一 `scan` 新产物使用 `schema_version=4`、
