...
- 状态：**v1.6.1 初步规范，非稳定发布**。
- 对应开发版本：**v1.6.1**；最近稳定标签：**v1.6.0**。
- 发布边界：本轮只推送 `Codex` 分支，不建立 `v1.6.1` 标签。
...
- 当前应用版本为 `1.6.1`。统一 `scan` 新产物使用 `schema_version=4`、
  `daisy-snapshot-v4` 和独立 `daisy-resume-v1`；冻结的 `full-scan／quick-scan` 入口继续
  生成 schema 3。元数据 profile 仍为 7，最低旧库读取器基线仍为 `v1.4.1`。
- v1.6.1 是初步修改版，不代表稳定发布质量；`v1.6.0` 仍是最近稳定标签。本轮不建立
  `v1.6.1` 标签，真实工具、特殊文件、超大档案和不同 Windows 显示环境仍需继续验证。
...
- v1.6.1 是 v1.6.0 数据契约上的 UI／交互版本：统一扫描新增 ExifTool／ffprobe 两个
  schema 4 冻结配置键，缺键默认 `true`；schema 3／4 DDL、Diff DDL、Reader 投影、
  resume contract 和最低读取器均不变。冻结 `full-scan／quick-scan` 参数集合保持不变。
...
- 当前顶栏固定为 `文件｜面板｜高级｜设置｜视图｜帮助`，菜单栏使用浅米黄色底色。
  「面板」包含「环境」「数据」和「硬盘」三个子菜单；色带下方固定一排显示 6 个等宽
  功能按钮。「高级」包含「工具路径」、扫描阶段独立哈希复检设置、
  动态「显示／隐藏命令预览」命令和「DAISY功能自检」。DBS-11 独立哈希抽验与
  统一 Full 的独立哈希抽验保留在高级菜单；数据核验不再提供哈希或格式范围滑块。
- GUI 的数据域只显示“扫描、对比、核验、数据库解析”4 个用户入口；旧 Full／Quick、
  哈希／格式核验和报告导出 TaskSpec 继续用于偏好迁移、恢复指针与脚本兼容，不重复显示。
- 所有功能模块按钮与设置页标题共用同一套六字名称；6 个入口始终保持单排，选中态不得
  改变按钮尺寸。GUI 中，值域严格为
  `{False, True}` 的表单项固定使用二态按钮：黄色表示关闭，按下后绿色表示启用；顶部
  设置不提供控件样式切换。扫描页的完整／快速使用两个
  固定模式按钮；未选择时不展开其它设置。Full 元数据使用 ExifTool／ffprobe 两个独立
  按钮；数据核验使用内置、ExifTool、ffprobe、7-Zip、RAW 深检五个同排按钮，开启后
  全量检查适用文件。预设和 timeout 处置等其它多值模式继续使用下拉菜单。
  RAW 深检受统一能力探测约束。根标签映射显示为文本输入，“添加目录”置于输入框上方；既有
  `--force`、`--map-root` 默认值和执行语义不变。只读下拉框未展开时拦截鼠标滚轮，滚轮
  只滚动页面，不改变选项。
- 普通按钮使用统一字符宽度、padding 和 8 px 水平间距；文件资源管理器按钮使用较小专用
  尺寸，顶部六入口使用较高专用尺寸。ENV 页按“检测状态 → 软件安装 → 报告目录”排列，
  六个检测状态按钮和五个安装按钮分别保持单排、等尺寸。
- 顶部「设置」菜单提供默认关闭且持久化的“任务完成提示音”。普通任务正常结束或
  “完成但需要检查”时异步播放一次；失败、暂停、保存退出、停止、依赖安装和物理盘检测
  准备步骤不播放。提示音失败只写日志，不改变退出码或报告。
- 任务队列成功、需检查、失败、停止或保存退出后重新展开设置；结束时若处于小窗模式，
...
|---|---|
| ENV-01 | 环境报告目录 |
| 扫描 | 扫描模式、生成方式、档案根目录、建库方式、（续传快照）、快照目录、（元数据工具）、（元数据范围）、NTFS标识、（内容哈希）、（重试范围） |
| 对比 | 基准快照、对比快照、差异目录、根标签映射、指纹降级 |
| 核验 | 封存快照、档案根目录、校验工具、报告目录 |
| 数据库解析 | 输入数据库、内容预设、内容模块、输出格式、报告目录 |
...
| 硬盘检测失败 | 展开并保留当前选择 | 展开并保留诊断 | 展开并保留诊断 |
| 开始识别数据库 | 收起 | 展开 | 展开 |
| 数据库识别成功 | 展开并刷新模块池 | 收起 | 收起 |
| 数据库识别失败 | 展开并清空旧模块 | 展开并保留诊断 | 展开并保留诊断 |

进度详情必须把 `errors`、`not_applicable` 和 `skipped` 分别呈现为“错误”“不适用”和
...
- v1.6.1 的仓库验收已覆盖多字体、3 档字号、1.0／1.25／1.5 Tk scaling，以及
  1840×1020、1440×900、1280×720、1280×960、1280×1024、1100×850；设置菜单另覆盖
  1920×1080、1600×900、1366×768。默认 1080P 下 6 个正式页面均无可见滚动；较小窗口
  或特大字号只有在内容超过视口与 2 px 几何容差后才显示 scrollbar，并可到达最后一项。
  固定按钮 UI 的正式页面交叉矩阵为 2 种本机候选字体 × 3 种 scaling × 3 档字号 ×
  3 种几何 × 6 个页面，共 324 组；更广窗口集合另行遍历全部现行与兼容 TaskSpec。
  顶部 6 个入口必须保持单排，因此 1024 px 不声明为正常支持宽度；窗口以实际工具栏最小
  宽度约束，不以裁切按钮伪装支持。该矩阵只证明当前仓库自动化结果，不等同于稳定发布。

### 12.5 偏好、关闭与管理员重启

- 顶部「设置」菜单持久化默认窗口大小、字体族、字号、任务完成提示音、
  最后功能页面、白名单非路径选项和手动工具路径。
  正常关闭或管理员重启后恢复页面与非路径选项，不保存档案／数据库／输出路径或硬盘选择。
...
| `recovery_scans` | `[]` | 最多 20 条受控 partial 恢复提示，不自动开始读取 |

旧配置中的 `binary_control_style` 与其它未知键一样被忽略，不会转成任务参数或再次写入；
非法字段逐项回退。`storage_list` 是 STG-11 内部步骤，不能
作为恢复页面。偏好文件不得包含档案路径、封存数据库路径、输出路径、硬盘编号、目录
...
- Python 已可运行时，「ENV-01 运行环境检测」会同时报告已发现工具的本机版本和
  全部缺失项。ENV-01 先用一排六个等尺寸状态按钮显示 ExifTool、ffprobe、7-Zip、
  PowerShell、smartctl 和 rawpy，再用一排五个等尺寸安装按钮提供 ExifTool、ffprobe、
  7-Zip、smartctl 与 rawpy／LibRaw。外部工具只在用户再次确认后通过固定 WinGet 白名单
  处理；rawpy 只在确认后通过当前 GUI 所属 Python 的固定 pip 白名单安装或更新，使用
  `only-if-needed` 依赖策略。PowerShell 不由 GUI 安装。
