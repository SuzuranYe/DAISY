...
- 顶部功能入口使用四字短名，设置页、进度、日志和报告使用完整功能名；6 个入口始终保持
  ...
  数据核验使用基本校验、ExifTool、ffprobe、7-Zip、rawpy/LibRaw 五个
  完整名称同排按钮。当前会话尚未收到完整环境清单时，核验区显示「需要先运行环境检测」，
  四个外部项目灰显，开始操作被参数预检拒绝；基本校验作为内置能力保持可见。环境检测后，
