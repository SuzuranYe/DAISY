...
- 状态：**v1.6.8 稳定生产规范**。
- 当前稳定生产版本与最新正式 Git 标签：**v1.6.8**；历史兼容稳定生产版本：**v1.4.1**。
...
- 设置正文初始展开，运行进度和运行日志正文初始收起。任务开始时收起设置并展开进度和日志；
  任务队列
  成功、需检查、失败、停止或保存并退出后等待 2 秒，只收起运行进度，运行日志保持展开并接管
  进度正文释放的高度，不自动展开设置。倒计时内用户手动切换进度时取消本次自动收起。依赖
  安装后的自动环境复检与硬盘检测完成时遵守同一规则。
...
v1.6.8 的权威配色源为《明日方舟_孤星_配色取样_修复版_v2.pptx》。GUI 只允许使用下列
...
- 功能栏在完整界面中固定显示；设置区、进度区和日志区可独立显示或隐藏。设置区默认展开，
  ...
  日志展开时，日志正文增加等于进度正文的高度；任务结束后的常见布局因此由日志接管原进度
  区域。命令预览
...
| 任务运行结束 | 不自动展开，保留手动状态 | 2 秒后收起 | 保持展开并接管进度正文高度 |
| 开始检测硬盘 | 收起 | 展开 | 展开 |
| 硬盘检测成功 | 保持并刷新硬盘选择 | 2 秒后收起 | 保持展开并接管高度 |
| 硬盘检测失败 | 保持，硬盘清单不更新 | 2 秒后收起并保留诊断 | 保持展开并保留诊断 |
...
- 硬盘信息登记检测开始时自动展示进度与日志。检测成功后弹窗并刷新硬盘池；检测结束 2 秒后
  收起进度并让日志接管释放高度，设置保持收起，用户需要选择硬盘时手动展开。检测失败同样
  保留日志诊断并执行进度收起规则。
