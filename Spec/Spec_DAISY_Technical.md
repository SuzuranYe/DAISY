...
- 状态：**v1.6.2 正式发布规范**。
- 对应稳定版本与标签：**v1.6.2**。
- 发布沿革：v1.6.1 仅为未打标签的阶段性修改，正式发布版本为 v1.6.2。
...
- `filename_layout_version=3` 表示 schema 4 的自描述命名合同：v1.6.0 已有带功能 token 的
  合格文件继续接受，v1.6.2 新建文件只写 `Full`/`Quick`；两者均以库内 pattern 和 manifest
  校验，不能只靠文件名猜测能力。
...
v1.6.2 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.2`。
...
- 当前应用版本为 `1.6.2`。统一 `scan` 新产物使用 `schema_version=4`、
