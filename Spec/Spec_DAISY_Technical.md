...
- 「结果目录提示」默认关闭。普通业务任务产出有效结果后，只将底部米黄色的
  「打开结果目录」按钮以绿色闪烁两次，再自动恢复，不抢焦点；启用该设置后才额外询问是否打开目录。
  底部操作从右向左固定为「开始／停止 → 暂停／继续 → 打开结果目录」，使用同一宽度、
  内边距和字号。同一个「开始」按钮在任务运行时原位变为橙色「停止」；扫描进入暂停请求态后，
  才在暂停按钮左侧显示「保存并退出」，暂停按钮原位改为「继续」。未使用独立的可见停止按钮。
  「重置会话」位于顶部功能栏标题区，运行期间禁用。
...
|---|---|
| 运行环境检测 | 环境报告目录 |
| 档案扫描建库 | 扫描模式、生成方式、档案根目录、建库方式、（续传快照）、快照保存目录、元数据（ExifTool/ffprobe 独立三态）、NTFS-ID、哈希（不采集／SHA-256）、（重试范围） |
