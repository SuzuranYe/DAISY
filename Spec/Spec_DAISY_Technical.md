...
- v1.4.2 定位为纯 UI 优化版本：增加只保留进度与停止控制的小窗运行视图；多项任务
  使用独立队列总进度条，并把当前任务阶段与本阶段工作量恢复为各自语义；
  左侧导航在锁定时保持原前景色，窄窗口优先保留开始／停止按钮，按钮统一
  提供悬停说明。设置、进度、日志和左侧导航可独立折叠，日志固定置于进度
  下方；左侧以减淡的绿、黄、红标出“环境”“数据库”“硬盘”三区，任务按
  `ENV-`／`DB-` 命名，并为后续存储设备模块预留 `STG-`，但不创建空任务页。
  顶部使用一笔向左右展开的铃兰耳朵线稿
  与 DAISY 组合标志，以 Palatino Linotype Italic 列出全称和版本号，并移除
  自定义快捷键和任务页提示徽标。标准菜单提供目录、退出、
  视图、关于与 GitHub 入口，命令预览默认关闭；底部操作按钮按宽度自动换行。
...
- Python 已可运行时，「ENV-00 环境监测」会同时报告已发现工具的本机版本和
...
可重复执行的回归测试位于 [`Script\Test`](../Script/Test/)；GUI 的“ENV-01 项目自检”
