...
| 对象 | 统一写法 | 说明 |
|---|---|---|
| 六个主功能 | 运行环境检测、档案扫描建库、档案快照对比、档案数据核验、档案数据解析、硬盘信息登记 | 页面标题使用完整名称；顶部入口依次使用「扫描建库、快照对比、数据核验、数据解析、硬盘登记、环境检测」六个四字短名 |
...
- 当前顶栏固定为 `文件｜功能｜视图｜设置｜高级｜帮助`，菜单栏使用浅米黄色底色，
  ...
  「设置」包含窗口大小、界面字体、工具路径、完成提示音和结果目录提示；恢复入口统一为
  顶部功能栏的「重置软件」；
...
- 「结果目录提示」默认关闭。普通业务任务产出有效结果后，只将底部米黄色的
  ...
  该按钮的常态背景、文字、悬停色和边框与顶部六个未选中功能入口完全一致。
  底部操作从右向左固定为「开始／停止 → 暂停／继续 → 打开结果目录」。三个常驻按钮与
...
- 普通窗口以 `1600×900` 为默认目标尺寸；设置中仍提供 `1920×1080` 和
  `1366×768`。Windows 进程启用 Per-Monitor V2
...
- v1.6.2 的发布验收覆盖多字体、3 档字号、1.0/1.25/1.5 Tk scaling，以及
  ...
  1600×900、1920×1080、1366×768。标准字号和 `1600×900` 默认窗口设置下，6 个正式页面
  的初始状态均无可见滚动；完整扫描等条件字段全部展开后可以产生真实滚动。较小窗口、
  展开的条件字段或特大字号只有在内容超过视口与 2 px 几何容差后才显示滚动条，并可到达
  最后一项。
  ...
  1100 px，窄窗口只压缩按钮内部横向留白，不缩写、裁切或换行四字入口。该矩阵只证明
...
| `window_size` | `[1600, 900]` | 普通窗口目标客户区；仍受当前工作区约束 |
...
- GUI 默认优先使用 `Microsoft YaHei UI`，并可在本机已安装的中文／系统候选字体间
  ...
  「收起设置」的右边界对齐。功能模块标题使用与运行进度、运行日志一致的黑色标题字，三色装饰线
...
- 安装队列完成后 GUI 刷新当前进程 PATH，并通过统一能力探测层重新检测外部工具和
  ...
  退出码宣称升级成功。
- 顶部功能栏的「重置软件」先要求确认，再恢复默认窗口、字体、提示音和结果目录提示，清除
