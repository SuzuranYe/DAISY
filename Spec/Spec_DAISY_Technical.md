...
- 状态：**v1.6.7 致命错误修复版规范**。
- 当前正式版本及最新 Git 标签：**v1.6.7**。
- 发布基线沿革：**v1.4.1 → v1.6.4 → v1.6.6 → v1.6.7**。v1.6.4 与 v1.6.6
  虽曾被指定为长期稳定基线，现均因 GUI 哈希功能确认失效而由 v1.6.7 取代；当前只支持 v1.6.7。
- 发布沿革：v1.6.1 仅为未打标签的阶段性修改，v1.6.2 首次收敛该阶段能力，v1.6.3
  ...
  进程退出异常及默认功能自检可能拖垮桌面的致命问题，不改变数据契约。
...
v1.6.7 的统一 Reader 只读支持 v1.4.1/schema 3 与当前 schema 4 封存快照，不原地
...
早期协议；Manifest 中的应用版本取当前 `APP_VERSION`，本版为 `1.6.7`。
...
- 当前应用版本为 `1.6.7`。统一 `scan` 新产物使用 `schema_version=4`、
