...
- 顶部功能入口与设置页、进度、日志和报告统一使用六字完整功能名；6 个入口始终保持
  ...
  档案数据解析的导出范围及
  HTML/XLSX/CSV/JSONL 输出格式均使用固定按钮，不显示复选框，且初始均不选择。
  RAW 深度校验受统一能力探测约束。对比页先等待基准与对比快照都完成选择，再显示默认未选的
  「单根目录／多根目录」；选择单根目录时自动对应，只有选择多根目录才展开根目录名配对。
  更换任一快照必须清除目录数量和旧配对，再从该章节继续。每组「基准根目录名／对比根目录名」
  按上下两行编辑；「添加配对」位于
...
- 扫描模式和生成方式需要改变条件字段时，GUI 先在未映射的新 `Frame` 中完成控件、字体和
  ...
  的按钮只保存值和更新预览，不重建表单。档案数据解析、快照对比和硬盘登记推进下一章节时
  使用同一离屏替换机制，并保留仍有效的上游值和滚动位置。
- 档案数据解析采用依赖链显露：没有数据库路径时只显示输入数据库及天青「选择」；路径已选但
  尚未成功识别时只增加科研蓝「解析数据库」；识别成功后只显示默认未选的导出范围。摘要或
  全部预设会自动确定数据模块并显示输出格式；自定义预设必须至少选择一个数据模块才显示输出
  格式；至少选择一种格式后才显示数据导出目录。数据库「选择」位于第一行，完整路径输入框
  独占第二行；不得让长路径与操作按钮在同一行竞争宽度。识别失败或任一必需选择未完成时，
  后续内容继续隐藏，底部「开始」保持禁用。
  点击「解析数据库」不得改变设置、进度或日志的展开状态；离屏重建表单时必须保持数据库路径
  和滚动位置，不得出现先清空后逐项闪现。模块按钮使用固定尺寸、按可用宽度换行并从左侧开始
  排列，不随容器横向拉伸。
- 六个主功能必须按真实依赖自上而下显露章节：扫描先选扫描模式、再选生成方式；核验先选
  数据库状态，已有数据库时再选路径状态；快照对比先选齐两份快照、再选目录数量；数据解析
  按数据库选择、数据库解析、导出范围、模块／格式和输出目录推进；硬盘登记先检测、再选盘、
  最后显示输出设置。环境检测没有前置用户输入，不得为追求形式而增加无意义的确认步骤。
...
可选配色来自用户补充色板，不改变核心色的语义，只负责大面积中性色、信息分层和进度区分：

| 补充色 | 色值 | 当前用途 |
|---|---|---|
| 暖白 | `#F0E7E2` | 窗口底、中性按钮与进度／日志内容面 |
| 科研蓝 | `#5E8CC0` | 工作流主操作与队列总进度 |
| 鼠尾草 | `#647B75` | 设置顶部区域、阶段进度与次要文字 |
...
一组矮按钮。进度与日志标题区右侧操作组使用 12 px 四边等距外边距，最右展开／收起按钮
...
- 功能栏、设置区、进度区和日志区可独立显示或隐藏，运行进度与运行日志默认
  ...
  三条进度分别使用科研蓝、鼠尾草和天青，进度槽统一使用石灰。队列、任务阶段和本阶段三行
  均固定保留百分比位置；确定进度显示整数百分比，当前阶段无法计算时显示「计算中」，结束后
  恢复并保留数值百分比。日志可打开为
...
- 「档案数据解析」先识别快照或 Diff，再按统一 Reader 能力选择数据模块。快照最多 15 个数据模块，
  ...
  操作行，解析成功后再按依赖逐章显露导出设置。「选择」与「解析数据库」同为标准按钮尺寸，分别使用
  天青输入色和科研蓝执行色。GUI 将内容选择命名为「导出范围」和「数据模块」，范围用按钮选择，
  数据模块按钮固定靠左排列；解析操作按阶段替换表单内容，但不得改变页面展开状态或滚动位置。
  导出范围与 HTML/XLSX/CSV/JSONL 格式均默认不选；选择范围后才显示可用模块，完成模块选择
  后才显示格式，选择至少一种格式后才显示输出目录。切换导出范围不得覆盖已经选择的格式；
...
- GUI 默认优先使用 `Microsoft YaHei UI`，并可在本机已安装的中文／系统候选字体间
  ...
  12 px 四边等距外边距；最右展开／收起按钮四周的可见空隙一致。
