...
- v1.4.2 定位为纯 UI 优化版本：增加只保留进度与停止控制的小窗运行视图；多项任务
  ...
  “环境”“数据库”“硬盘”提供三个任务下拉菜单；下拉项具有逻辑分隔、主题色
  悬停和当前任务高亮，色带下方另设可折叠、可换行且同步选中状态的任务按钮
  菜单。环境检测使用 `ENV-01`；完整扫描与快速扫描分别使用 `DB-11`、`DB-12`，
  并与 `DB-21`、`DB-31`、`DB-32`、`DB-41` 及维护测试 `DB-91` 一同归入
...
可重复执行的回归测试位于 [`Script\Test`](../Script/Test/)；GUI 的“DB-91 数据库自检”
