r"""Script_DAISY_Module_Quick_Scan：快速档案扫描——只登记文件信息的轻量快照。

与完整档案扫描采用同一快照格式（同 DDL、内嵌事件与清单），但**完全
...
用法：
  python .\Script\Script_DAISY_CLI.py quick-scan --root "label=D:\档案" --output-dir .\Snapshots
"""
