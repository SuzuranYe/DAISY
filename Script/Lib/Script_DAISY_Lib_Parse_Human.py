...
:root{{--bg:#f0e7e2;--card:#fff9ef;--ink:#171614;--muted:#647b75;--line:#a89f98;--accent:#5e8cc0;--warn:#de5123}}
...
header{{padding:28px clamp(18px,4vw,54px);background:#2c3240;color:#fff9ef}}header h1{{margin:0 0 6px;font-size:clamp(24px,4vw,38px)}}header p{{margin:0;color:#f0e7e2}}
.layout{{display:grid;grid-template-columns:220px minmax(0,1fr);gap:22px;max-width:1500px;margin:0 auto;padding:22px}}nav{{position:sticky;top:18px;align-self:start;background:var(--card);border:1px solid var(--line);border-radius:12px;padding:14px}}nav a{{display:block;color:var(--accent);padding:6px 8px;text-decoration:none;border-radius:6px}}nav a:hover{{background:#80afc3;color:#171614}}main{{min-width:0}}
...
.facts{{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px}}.fact{{padding:10px;border-radius:8px;background:#f0e7e2}}.fact span{{display:block;color:var(--muted);font-size:12px}}.fact strong{{display:block;overflow-wrap:anywhere}}
...
.table-wrap{{overflow:auto;max-height:620px;border:1px solid var(--line);border-radius:8px}}table{{border-collapse:collapse;min-width:100%;background:#fff9ef}}th,td{{padding:8px 10px;border-bottom:1px solid var(--line);border-right:1px solid #a9a397;text-align:left;vertical-align:top;white-space:pre-wrap;overflow-wrap:anywhere}}th{{position:sticky;top:0;background:#a9a397;z-index:1}}th span,th code{{display:block}}th code{{font-size:11px;color:var(--muted)}}td{{max-width:520px}}
...
@media print{{body{{background:#fff9ef}}nav,.filter-label{{display:none}}.layout{{display:block;max-width:none;padding:0}}header{{background:#fff9ef;color:#171614;padding:0 0 16px}}header p{{color:#647b75}}section,.hero,.identity{{break-inside:avoid;border-color:#a89f98}}.table-wrap{{max-height:none;overflow:visible}}th{{position:static}}}}
</style>
</head>
<body>
